...
/*
        pref = activity!!.getSharedPreferences(Constants.PREF_NAME, 0)

        bestScore = pref.getInt
        (
            Constants.EASY_HIGH_KEY,
                        ...
                        override fun onTick(millisUntilFinished: Long) {
                            if (isPaused || isCancelled) {
                                ...
                                if (count == Constants.EASY_NO_OF_CARDS) {
                                    b!!.putString("Data", "win")
                                    time =
                                        ((Constants.EASY_TIME - millisUntilFinished) / Constants.TIMER_INTERVAL).toInt()
                                    b!!.putInt("Time", time)
                                    cancel()
                                    onFinish()
                                }
